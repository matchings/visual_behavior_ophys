...
class VisualBehaviorSutterDataset(object):

    def __init__(self, expt_session_id, expt_dir, expt_info_df, filter_masks_and_traces=True):
        ...
        self.expt_info_df = expt_info_df
        self.expt_session_id = int(expt_session_id)
    ...
    def get_expt_info(self):
        tmp = self.expt_info_df
        session_id = self.expt_session_id
        expt_info = tmp[tmp.session_id == session_id]
        self.expt_info = expt_info
    ...
    def get_pkl_path(self):
        pkl_file = [file for file in os.listdir(self.expt_dir) if file.endswith(str(self.metadata['mouse_id']) + '.pkl')]
    ...
    def get_running_speed(self, smooth=True):
        ...
        timestamps = self.sync['visualFrames']['timestamps']
        return self.running_speed, timestamps


    def get_sync_path(self):
        expt_id = self.expt_dir.split('/')[-2]
        sync_file = [file for file in os.listdir(self.expt_dir) if
                     file.startswith(expt_id + '-') and file.endswith('.h5') and 'extracted_traces' not in file]
        self.sync_path = os.path.join(self.expt_dir, sync_file[0])
    ...
    def get_filter_parameters(self):
        ...
        self.filter_params = rm.get_standard_filter_parameters(zoom,layer)
        return self.filter_params


    def get_filtered_roi_indices(self):
        self.filtered_roi_inds = rm.get_filtered_indices(self.roi_metrics, self.filter_params)
    ...
    def get_neuropil_masks(self):
            ...
            if self.filter_masks_and_traces:
                mask_shape = unfiltered_neuropil_masks[0].shape[0]
                filtered_neuropil_masks = np.empty((len(self.filtered_roi_inds),mask_shape,mask_shape))
    ...
    def get_roi_masks(self):
            ...
            if self.filter_masks_and_traces:
                mask_shape = unfiltered_roi_masks[0].shape[0]
                filtered_roi_masks = np.empty((len(self.filtered_roi_inds), mask_shape, mask_shape))
                for filtered_roi_ind, unfiltered_roi_ind in enumerate(self.filtered_roi_inds):
    ...
    def get_error_and_r_values(self):
        try:
            ...
            print('no error and r_values for', self.expt_session_id)
        if self.filter_masks_and_traces:
            filtered_error = np.empty((len(self.filtered_roi_inds)))
            filtered_r_value = np.empty((len(self.filtered_roi_inds)))
            for filtered_ind, unfiltered_ind in enumerate(self.filtered_roi_inds):
            ...
            self.neuropil_error = unfiltered_error
            self.neuropil_r_value = unfiltered_r_value
    ...
    def extract_traces_from_hdf5(self,file_key):
        ...
        if self.filter_masks_and_traces:
            filtered_traces = np.empty((len(self.filtered_roi_inds), unfiltered_traces.shape[1]))
            for filtered_ind, unfiltered_ind in enumerate(self.filtered_roi_inds):
    ...
    def get_ophys_trace_timestamps(self):
        timestamps = self.sync['2PFrames']['timestamps']
        return timestamps


    def get_dff_traces(self):
        self.dff_traces = self.extract_traces_from_hdf5('dff_traces')
        timestamps = self.get_ophys_trace_timestamps()
        return self.dff_traces, timestamps


    def get_neuropil_traces(self):
        self.neuropil_traces = self.extract_traces_from_hdf5('neuropil_traces')
        timestamps = self.get_ophys_trace_timestamps()
        return self.neuropil_traces, timestamps


    def get_raw_traces(self):
        self.raw_traces = self.extract_traces_from_hdf5('roi_traces')
        timestamps = self.get_ophys_trace_timestamps()
        return self.neuropil_traces, timestamps


    def get_baseline_traces(self):
        self.baseline_traces = self.extract_traces_from_hdf5('baseline_traces')
        timestamps = self.get_ophys_trace_timestamps()
        return self.baseline_traces, timestamps


    def get_neuropil_corrected_traces(self):
        self.neuropil_corrected_traces = self.extract_traces_from_hdf5('corrected_traces')
        timestamps = self.get_ophys_trace_timestamps()
        return self.neuropil_corrected_traces, timestamps
        ...
        if stimulus_type == 'image':
            for trial in df.index:
                if df.change_image[trial] != 0:  # if its a change trial
                    ...
                    i += 1
            self.stim_table = pd.DataFrame(change_list, columns=['change_trial', 'total_trial', 'change_frame', 'change_time',
                'initial_code', 'change_code','initial_image','change_image','trial_type', 'behavioral_response', 'behavioral_response_type'])
            ...
            for trial in df.index:
                if df.trial_type[trial] != 'aborted':  # if its a change trial
                    ...
                    i += 1
            self.stim_table = pd.DataFrame(change_list, columns=['change_trial', 'total_trial', 'change_frame', 'change_time',
                'initial_code', 'change_code','trial_type', 'behavioral_response', 'behavioral_response_type'])
