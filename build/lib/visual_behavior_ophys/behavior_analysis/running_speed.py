...
def get_run_df(expt_session_id):
...
def plot_running_speed_changes(run_df, session_id, xlim=None, ax=None):
    ...
    """
    running_speed = run_df.running_speed.values[0]
    timestamps = run_df.timestamps.values[0]
    ...
    accelerating[np.logical_not(run_df.accelerating_mask.values[0])] = np.nan
    decelerating[np.logical_not(run_df.decelerating_mask[0])] = np.nan
    stationary[np.logical_not(run_df.stationary_mask[0])] = np.nan
    if ax is None:
        fig,ax = plt.subplots(figsize=(15,4))
